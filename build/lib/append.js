...
export async function getFullComponent(name) {
    const component = await getFile('mjml', resolve(__dirname, `components\\${name}`));
    return component;
}
export async function getSections(fullComponent) {
    ...
    return [
        ...
        body === null || body === void 0 ? void 0 : body.shift(),
        media280 ? media280.shift() : '',
        regularStyles ? regularStyles.shift() : ''
    ];
}
export async function beautifySections(sections) {
...
export function indent(sections) {
    let media480 = sections[0];
    let body = sections[1];
    let media280 = sections[2];
    let regularStyles = sections[3];
...
export async function insertSections(section, mjml, type, name) {
    let finder;
    let replacer;
    if (type === 'media480') {
        finder = /( *)(?=<\/mj-style>)/;
        ...
        finder = /(\s*)<\/mj-body>/g;
        mjml = await format(mjml.replace(finder, `\n\n<!--START ${name === null || name === void 0 ? void 0 : name.toUpperCase()}-->\n\n` + section + `\n<!--END ${name === null || name === void 0 ? void 0 : name.toUpperCase()}-->\n\n<\/mj-body>`), { parser: 'html', singleAttributePerLine: true, bracketSameLine: true });
    }
    return mjml;
}
...
export async function importComponents(commandParameter, name) {
    if (typeof commandParameter === 'string') {
        const components = splitComponents(commandParameter);
